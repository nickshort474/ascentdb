...
export default class CommunityMessageComp extends Component{
	...
	componentWillMount(){
		...
		let replyRef = this.firestore.collection("CommunityMessages").doc(this.props.messageID).collection("MessageReplies").orderBy("date");
		...
		return(
			<div>
				<section className="well msgWell" style={this.contentStyle}>
						...
						<div className="row">
							<Link to={`/MessageReply/${this.state.messageUser}/${this.state.messageID}/Community`} className="msgCompStyle">Reply</Link>
						</div>
